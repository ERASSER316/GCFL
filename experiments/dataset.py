...
def gen_classes_per_node(dataset, num_users, classes_per_user=2, high_prob=0.6, low_prob=0.4): #high_prob=0.6, low_prob=0.4
    ...
    count_per_class = (classes_per_user * num_users) // num_classes #每个类别必须分配给多少个客户端
    class_dict = {}
...
def gen_data_split(dataset, num_users, class_partitions):
    ...
    return user_data_idx    #[num_users][num_samples]
...
def gen_random_loaders(data_name, data_path, num_users, bz, classes_per_user, LowProb):
    ...
    for i, d in enumerate(datasets):
        ...
        if i == 0: # train set
            ...
            loader_params['shuffle'] = True #训练集需要打乱对应类别中的样本
        usr_subset_idx = gen_data_split(d, num_users, cls_partitions)

        if i == 0:
            train_sample_count = [len(i) for i in usr_subset_idx]   #存储每个用户分到了多少个训练样本，[num_users][num_samples]
        elif i == 1:
            eval_sample_count = [len(i) for i in usr_subset_idx]   #存储每个用户分到了多少个验证样本
        else:
            test_sample_count = [len(i) for i in usr_subset_idx]   #存储每个用户分到了多少个测试样本
        # create subsets for each client
        subsets = list(map(lambda x: torch.utils.data.Subset(d, x), usr_subset_idx)) #指定样本索引的对象
