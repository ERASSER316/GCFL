...
def train(data_name: str, data_path: str, classes_per_node: int, num_nodes: int, fraction: float,
    ...
    client_sample_count = [train_sample_count[i] + eval_sample_count[i] + test_sample_count[i] for i in
                           range(len(train_sample_count))]      #将每个客户端的训练集、验证集、测试集单独放在一起，便于后面便利
    # -----------------------------------------------#

    small_rep_dim = 500     #设置从小模型抽取的特征维度
    ...
    with open(orininal_file, 'w', newline='') as file:
        ...
        for step in step_iter:  # step is round
            ...
            LNs = defaultdict() # collect small_local_model ，for aggregation
            ...
            for c in select_nodes:
                ...
                for i in range(epochs):
                    ...
                    for j, batch in enumerate(nodes.train_loaders[node_id]):
                        ...
                        _, small_rep = net_small(img, torch.randn(len(label), small_rep_dim).to(device))
                        _, large_rep = net_large(img, torch.randn(len(label), 500).to(device))
                        #small_rep:[batchsize,250]
                        #large_rep:[batchsize,500]
                        rep = torch.cat((small_rep,large_rep), dim=1) #rep:[batchsize,1000]
                        m_rep = net_proj(rep) #m_rep:[batch,500]  大模型小模型的知识融合
                        #前面设置small_rep_dim=500，那这不相当于直接把m_rep赋值给了m_rep_small这个小模型了吗？
                        m_rep_small = m_rep[:, :small_rep_dim] 

                        large_pred, _ = net_large(img, m_rep)   #使用融合后的知识得到预测值，但是这此的结果只走了fc3这一个线性层[500,out_dim]，得到类别预测的vector
                        small_pred, _ = net_small(img, m_rep_small)

                        large_loss = criteria(large_pred, label)    #计算混合特征输入的loss
                        ...
                        #最终通过两个最终的loss去更新一个客户端中的三个模型的权重
                        optimizer_large.step()
                        optimizer_small.step()
                        # optimizer_alpha.step()
                        optimizer_proj.step()

                #一个客户端的三个本地模型在单轮本地训练中得到优化

                # collect local NN parameters
                #将优化后的权重赋值给改客户端原始权重，更新
            ...
            #计算本轮选择的所有客户端在10轮本地训练中的大小模型各自的平均准确率
            ...
            #权重的计算使用该客户端的所有训练集、测试集、验证集之和作为该客户端的总样本数
